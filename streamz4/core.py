...
import streamz4.ndeque_v2 as ndeque
import streamz4.pdeque as pdeque
...
class sliding_time_window_pandas_scheduled(Stream):
    def __init__(self, child, schedule, headings, **kwargs): #def __init__(self, schedule, headings,child, **kwargs):
        self.schedule = schedule #kwargs['schedule'] #schedule, headings,
        self.headings = headings #kwargs['headings']
        self.buffer = pdeque.Pdeque(window_size=-1,batch_time=-1,interval=-1,schedule=self.schedule, headings=self.headings)
        Stream.__init__(self, child, **kwargs)
...
class timed_window_pandas(Stream):
    def __init__(self, interval, e, child, **kwargs): #  def __init__(self, interval, e, headings,child, **kwargs):
